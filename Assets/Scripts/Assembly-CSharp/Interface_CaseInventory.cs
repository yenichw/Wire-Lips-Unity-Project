...
public class Interface_CaseInventory : MonoBehaviour
{
	...
	private void Start()
	{
		...
		for (int j = 0; j < objectCase.items.Length; j++)
		{
			...
			casesCaseItem[j].GetComponent<RectTransform>().anchoredPosition = new Vector2(num, -22f);
		...
		base.transform.Find("InventoryCase").GetComponent<RectTransform>().sizeDelta = new Vector2(num + 34.5f, 95f);
	...
	private void Right()
	{
		...
		else if (itemChange > objectCase.items.Length - 1)
		{
			itemChange = 0;
		}
		ChangeItem();
	...
	private void Left()
	{
		...
		{
			itemChange = objectCase.items.Length - 1;
		}
		ChangeItem();
	}
